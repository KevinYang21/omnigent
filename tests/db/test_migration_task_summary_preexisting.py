...
during a hotfix — the automatic startup upgrade to the current head aborts
with::
...
def _script_head(uri: str) -> str:
    """The migration graph's single head — the upgrade target at startup."""
    from alembic.script import ScriptDirectory

    head = ScriptDirectory.from_config(_build_alembic_config(uri)).get_current_head()
...
def test_upgrade_with_preexisting_task_summary_column(tmp_path: Path) -> None:
    ...
    assert _get_current_db_revision(engine) == _script_head(uri), (

...
def _localhost_env(extra: dict[str, str]) -> dict[str, str]:
    ...
    for name in ("HTTP_PROXY", "HTTPS_PROXY", "http_proxy", "https_proxy"):
        env.pop(name, None)
    # A shell that itself runs inside an omnigent runner (agent sandboxes)
    # leaks runner identity/config vars that would rebind the spawned server
    # and runner to the OUTER deployment. This test's world is fully local.
...
def test_cold_resume_resumes_history_when_large_item_page_500s(
    ...
    # also runs short `claude -p "/model" ...` capability probes through this
    # stub, so append one line per invocation (args joined by the unit
    # separator) rather than overwrite — the assertion below picks out the
    ...
    def _terminal_launch_argv() -> list[str] | None:
        """The interactive claude launch's argv, or None if not yet seen.

        Model/capability probes pass ``-p``; the terminal-as-agent launch
        is interactive (no ``-p``).
        """
        if not argv_file.exists():
            return None
        for line in argv_file.read_text().splitlines():
            args = line.split("\x1f")[:-1]
            if args and "-p" not in args:
                return args
        return None
    ...
    try:
        ...
        # Binding may do terminal-bootstrap work inline; give it CI headroom.
        _http.patch(
            f"{base_url}/v1/sessions/{session_id}",

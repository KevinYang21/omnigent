...
async def _auto_create_codex_terminal(
    ...
    *,
    ...
    try:
        ...
        from omnigent.codex_native_bridge import (  # noqa: FlagLocalImports
    ...
    if (
        launch_config.external_session_id is None
        and thread_start_timeout_seconds is not None
        and not _codex_launch.login_required
    ):
        try:
            ...
            # failure must not strand the terminal before its forwarder owns it.
            _logger.warning(
                "Could not publish Codex startup timeout for session %s; "
                "using the legacy executor wait",
                session_id,
                exc_info=True,
            )

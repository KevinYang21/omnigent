...
class SqlAlchemyAgentStore(AgentStore):
    ...
    def repoint_session_agents(
        ...
        session-scoped row whose key still lives under *source_agent_id* (or
        under *previous_bundle_location*'s prefix — legacy rows keep an
        ``ag_``-prefixed physical key) but differs from the current location is
        a stale, uncustomized clone. Per-session customization re-keys the
        ...
        :returns: Ids of session-scoped clone rows whose cached bundle may be
            ...
            *bundle_location*.
        """
        # ``autoescape`` neutralizes ``%``/``_`` wildcards in ids.
        prefixes = {f"{source_agent_id}/"}
